...
**文件**: `internal/middleware/auth.go`（后续会有 cors.go、rate_limit.go、logging.go 等）
...
**文件**: `internal/handler/user_handler.go`、`internal/handler/response.go`
...
**文件**: `internal/pkg/jwt/jwt.go`、`internal/pkg/hash/password.go`、`internal/pkg/errcode/errcode.go`
...
### 11. Storage（对象存储层，待实现）

**目录**: `internal/storage/`

Storage 封装 MinIO 操作，处理文件的实际上传、下载、删除。

文件不存 MySQL（数据库只存元数据：文件名、大小、哈希等），真正的文件内容存在 MinIO 里。

```
上传文件:
  Service → Storage.PutObject(bucket, key, fileBytes)
         → MinIO 存储文件
         → 返回 storage_key

下载文件:
  Service → Storage.GetPresignedURL(bucket, key, expiry)
         → MinIO 生成临时下载链接
         → 前端直接从 MinIO 下载
```
...
func main() {
    ...
    // cfg 包含：Server.Port, MySQL.DSN, Redis.Addr, JWT.Secret ...

    // ── 第2步：建立数据库连接 ──
    db, _ := database.InitMySQL(cfg.MySQL)    // MySQL 连接池
    rdb, _ := database.InitRedis(cfg.Redis)   // Redis 连接

    // ── 第3步：自底向上组装依赖 ──
    //
    // Repository 层（需要数据库连接）
    userRepo := repository.NewUserRepo(db)

    // Service 层（需要 Repository + 配置）
    userService := service.NewUserService(userRepo, cfg.JWT.Secret, cfg.JWT.ExpireHour)

    // Handler 层（需要 Service）
    userHandler := handler.NewUserHandler(userService)

    // ── 第4步：注册路由 ──
    r := gin.Default()
    router.Setup(r, userHandler, cfg.JWT.Secret)
    // Setup 内部：注册 URL → Handler 映射 + 挂载中间件
...
**组装顺序很重要**：必须从最底层（Repository）开始创建，因为上层依赖下层。

```
db → userRepo → userService → userHandler → router
 │                                               │
 数据库连接                                      注册到 Gin 引擎
```
